...
async def forward(c,m):
	...
	if check:
		...
		if m.text:
			...
			return await c.send_message(chat_id, text, reply_to_message_id=thread_id, reply_markup=button)
		if m.caption:
			with open('temp2.txt', 'w') as f:
		...
		if m.photo:
			await c.send_photo(chat_id=chat_id, photo=file, caption=caption, reply_to_message_id=thread_id, reply_markup=button)
		elif m.document:
			await c.send_document(chat_id=chat_id, document=file, caption=caption, reply_to_message_id=thread_id, reply_markup=button)
		elif m.video:
			await c.send_video(chat_id=chat_id, video=file, caption=caption, reply_to_message_id=thread_id, reply_markup=button)
		elif m.animation:
			await c.send_animation(chat_id=chat_id, animation=file, caption=caption, reply_to_message_id=thread_id, reply_markup=button)
		elif m.audio:
			await c.send_audio(chat_id=chat_id, audio=file, caption=caption, reply_to_message_id=thread_id, reply_markup=button)
		elif m.sticker:
